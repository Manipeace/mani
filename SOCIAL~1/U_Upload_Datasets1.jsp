...
<%@page
    ...
    try{
	   ...
	   String Title,Date1,Affiliations,Description,Response,Victims,Sponsor,Type,Category,Sources_Of_Attack,RID;
       ...
       for(int i=1; i<=sheet.getLastRowNum(); i++){  //points to the starting of excel i.e excel first row
           row = (Row) sheet.getRow(i);  //sheet number
				 ...
				 if( row.getCell(0)==null) { RID = "0";}  //suppose excel cell is empty then its set to 0 the variable
				  ...
				  if( row.getCell(2)==null) { Date1 = "0";}
				  else Date1 = row.getCell(2).toString();
				  
				   if( row.getCell(3)==null) { Affiliations = "0";}  
                  else Affiliations = row.getCell(3).toString();   
				  
				  if( row.getCell(4)==null) { Description = "0";}
				  else Description = row.getCell(4).toString();
				  
				  if( row.getCell(5)==null) { Response = "0";}
				  else Response = row.getCell(5).toString();
				  
				  if( row.getCell(6)==null) { Victims = "0";}
				  else Victims = row.getCell(6).toString();
				  
				  if( row.getCell(7)==null) { Sponsor = "0";}
				  else Sponsor= row.getCell(7).toString();
				  
				  if( row.getCell(8)==null) { Type= "0";}
				  else Type = row.getCell(8).toString();
				  
				  if( row.getCell(9)==null) { Category= "0";}
				  else Category = row.getCell(9).toString();
				  
				  if( row.getCell(10)==null) { Sources_Of_Attack= "0";}
				  else Sources_Of_Attack = row.getCell(10).toString();
			...
			stss.executeUpdate("insert into  dsdetails(Title,Date1,Affiliations,Description,Response,Victims,Sponsor,Type,Category,Sources_Of_Attack,hcode,RID) values ('"+Title+"','"+Date1+"','"+Affiliations+"','"+Description+"','"+Response+"','"+Victims+"','"+Sponsor+"','"+Type+"','"+Category+"','"+Sources_Of_Attack+"','"+h1+"','"+RID+"')"); 
